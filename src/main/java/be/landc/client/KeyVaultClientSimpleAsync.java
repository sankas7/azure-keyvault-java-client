 ...
 */
public class KeyVaultClientSimpleAsync {
    /**
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        SecretAsyncClient secretAsyncClient = new SecretClientBuilder()
            .vaultUrl("https://pramitkeyvault.vault.azure.net/")
            .credential(new DefaultAzureCredentialBuilder().build())
            .buildAsyncClient();

        // Let's create a secret holding bank account credentials valid for 1 year. if the secret
        // already exists in the key vault, then a new version of the secret is created.
        secretAsyncClient.setSecret(new KeyVaultSecret("MyPassword", "f4G34fMh8v")
        ...
        secretAsyncClient.getSecret("MyPassword").subscribe(secretResponse ->
        ...
        secretAsyncClient.setSecret("MyPassword", "bhjd4DDgsa").subscribe(secretResponse ->
            System.out.printf("Secret is created with name %s and value %s %n", secretResponse.getName(), secretResponse.getValue()));

        Thread.sleep(2000);

        // The bank account was closed, need to delete its credentials from the key vault.
        secretAsyncClient.beginDeleteSecret("MyPassword")
        ...
        secretAsyncClient.purgeDeletedSecretWithResponse("MyPassword").subscribe(purgeResponse ->
